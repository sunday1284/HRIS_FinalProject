		...
		<table class="table table-striped table-bordered table-hover" id="empTable">
			...
			<tbody>
				<c:forEach items="${empList}" var="x">
					...
					<tr>
						<td>
							<img src="${pageContext.request.contextPath}${webPath}" style="width: 40px; height: 40px; object-fit: cover; border-radius: 4px;"
						...
						<td>${x.job != null && x.job.jobName != null ? x.job.jobName : '-'}</td>
						<td>${x.position != null && x.position.positionName != null ? x.position.positionName : '-'}</td>
						<td>
