					...
					<table class="table table-bordered table-hover align-middle"
						...
						<tbody id="appointmentRows">
							<tr>
								...
								<td>
									<select class="form-control empSelect" name="appointmentList[0].empId" id="employee">
										...
										<c:forEach items="${empList}" var="x">
											<option value="${x.empId}" data-teamId="${x.teamId}" data-departmentid="${x.departmentId}" class="employee-option">
								...
								<td>
									<select name="appointmentList[0].newDepartmentId" class="form-control" id="departmentSub">
								...
								<td>
									<select name="appointmentList[0].newTeamId" class="form-control teamSelectTarget" id="teamSub">
										<option value="">팀 선택</option>
										<c:forEach items="${teamList}" var="team">
											<option value="${team.teamId}" class="teamSub-option" data-departmentIdSub="${team.departmentId}">${team.teamName}</option>
										</c:forEach>		
									</select>
								</td>
								<td>
...
<script>
	$(document).ready(function () {
	    ...
	    $(document).on("change", ".empSelect", function () {
	        ...
	        row.find(".empId").val("");
	        row.find(".name").val("");
	        row.find(".deptSelect").val("");
	        row.find(".departmentName").val("");
	        row.find(".prevDepartmentId").val("");
	        row.find(".teamSelect").val("");
	        row.find(".teamName").val("");
	        row.find(".prevTeamId").val("");
	        row.find(".rankName").val("");
	        row.find(".prevRankId").val("");
	        row.find(".jobName").val("");
	        row.find(".prevJobId").val("");
	        row.find(".positionName").val("");
	        row.find(".prevPositionId").val("");
	            ...
	            $.ajax({
	                ...
	                success: function (data) {
	                    if (data) {
	                        ...
	                        row.find(".deptSelect").val(data.department.departmentId); // ✅ select 값 설정
	                        row.find(".departmentName").val(data.department.departmentName); // ✅ 텍스트용 input엔 name
	                        row.find(".prevDepartmentId").val(data.department.departmentId);
	                        row.find(".teamSelect").val(data.team.teamId);
	...
	$("#department").change(function () {
		...
		$("#employee option.employee-option").each(function () {
			const empDeptId = $(this).data("departmentid");
			if (selectedDeptId === "" || empDeptId == selectedDeptId) {
				$(this).show();
			} else {
				$(this).hide();
			}
	...
	$("#team").change(function () {
		...
		const selectedDeptId = $("#department").val();

		$("#employee option.employee-option").each(function () {
			const empTeamId = $(this).data("teamid");
			const empDeptId = $(this).data("departmentid");

			if (selectedTeamId === "") {
				// 팀 선택 안한 경우 → 부서 기준 필터링 유지
				if (selectedDeptId === "" || empDeptId == selectedDeptId) {
					$(this).show();
				} else {
					$(this).hide();
				...
				// 팀 선택된 경우 → 팀 기준으로만 필터링
				if (empTeamId == selectedTeamId) {
					$(this).show();
				} else {
					$(this).hide();
				}
	...
	function fetchFilteredData() {
		...
		$.ajax({
			...
			dataType: "json",
			success: function (data) {
// 				console.log(data); // ← 필요에 따라 화면 렌더링 가능
			},
			error: function () {
	...
	document.getElementById("departmentSub").addEventListener("change", filterTeams);

	function filterTeams() {
		var selectedDept = document.getElementById("departmentSub").value;
		var teamSelect = document.getElementById("teamSub");
		var teamOptions = document.querySelectorAll(".teamSub-option");
		...
		teamOptions.forEach(function (option) {
			...
			console.log("Option Dept ID:", option.dataset.departmentIdSub);
			if (selectedDept === "" || option.dataset.departmentidsub === selectedDept) {
	...
	// 최초 로딩 시 팀은 전체 보이고, 직원은 숨김
	$(document).ready(function () {
		$("#team option.team-option").show();
