$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(response) {
			...
			// FancyTree 초기화 후, 트리 노드 확장 및 경영지원본부 자동 활성화
			var tree = $("#orgTree").fancytree("getTree");
			if (tree) {
			    // 레벨 3(팀 노드)까지 모두 확장
			    tree.visit(function(node) {
			        if (node.getLevel() <= 1) {  // 레벨 1: 본부, 2: 부서, 3: 팀
			            node.setExpanded(true);
			        }
			    });

			    // "경영지원본부" 이름을 가진 부서 노드를 찾아 활성화 (대소문자 및 공백 주의)
			    tree.visit(function(node) {
			        if (node.data && node.data.type === 'department' &&
			            node.data.departmentName.trim() === '경영지원본부') {
			            node.setActive(true);
			        }
			    });
			}
			...
			// 예: "팀 노드까지만 열기" 버튼
			$("#openToTeamBtn").on("click", function () {
			    $("#orgTree").fancytree("getTree").visit(function (node) {
			        // node.getLevel()은 1부터 시작해 (1: 본부, 2: 부서, 3: 팀, 4: 사원 ...)
			        if (node.getLevel() <= 1) {
			            node.setExpanded(true); // 팀까지만 열기
			        } else {
			            node.setExpanded(false); // 팀 이하(예: 사원)는 닫기
			        }
			    });
			});
