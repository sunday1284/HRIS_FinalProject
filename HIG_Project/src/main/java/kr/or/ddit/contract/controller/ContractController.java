...
public class ContractController {
    ...
    public String register(@ModelAttribute("contract") ContractVO contract, Model model) {
        service.insertContract(contract);
        model.addAttribute("success", true);  // 성공 플래그
        List<ContractVO> unContractList = service.unContractList();
        model.addAttribute("unContractList", unContractList);
        return "redirect:/contract/List";  // 그대로 등록 화면으로 forward
    }
