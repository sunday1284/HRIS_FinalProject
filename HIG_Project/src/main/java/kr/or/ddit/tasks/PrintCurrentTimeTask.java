...
//	@Scheduled(initialDelay = 3000, fixedDelay = 2000)
	@Scheduled(cron = "00 09 10 * * MON-FRI") // 평일 실행 (현재 테스트중)
	public void chcekAnnual() {
	    ...
	    for(AnnualHistoryVO AH : AHList) {
	        if ("승인".equals(AH.getStatus())) {
	            ...
	            // ✅ 중복 여부 체크
	            if (AHservice.existsAttendance(AH.getEmpId(), formattedToday)) {
	                continue; // 이미 기록 있음 → 스킵
	            }
...
//	@Scheduled(initialDelay = 5000, fixedDelay = 5000)
	...
	// 10시 10분부터 2분당 한번씩
	@Scheduled(cron = "0 10-59/1 10 18 * ?") 
	public void sendSalay() {
		 qrWebSocket.broadcastMessage("오늘은 급여 지급일입니다");
	}
