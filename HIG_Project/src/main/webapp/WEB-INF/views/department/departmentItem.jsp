...
<div class="container">
   <div class="input-container">
      <input type="text" id="itemCode" placeholder="부서 코드"> <input
         type="text" id="itemName" placeholder="부서 이름"> <input
         type="text" id="itemDescription" placeholder="부서 설명">
...
<script>
let departmentMangeList = [];
<c:forEach items="${departmentMangeList}" var="x">
    departmentMangeList.push({
        departmentCode: "${x.departmentCode}",
        departmentName: "${x.departmentName}",
        departmentInfo: "${x.departmentInfo}",
        departmentStatus: "${x.departmentStatus}"
    });
</c:forEach>;
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    departmentMangeList.forEach(item => {
        let newItem = createItemElement(item.departmentCode, item.departmentName, item.departmentInfo, item.departmentStatus);
        let newRow = createTableRow(item.departmentCode, item.departmentName, item.departmentInfo);

        if (item.departmentStatus === "N") {
...
function saveItem() {
    ...
    // 서버에 항목 저장 요청
    fetch('/department/Insert', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({ departmentCode: code, departmentName: name, departmentInfo: description, departmentStatus: "N" })
    })
...
function addItem(button, code, name, description) {
    ...
    fetch('/department/Update', {
        ...
        body: JSON.stringify({ departmentCode: code, departmentName: name, departmentInfo: description, departmentStatus: "Y" })
    })
    .then(response => response.json())
