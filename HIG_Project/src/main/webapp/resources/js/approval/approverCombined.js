		...
		$.getJSON(ctx + '/approval/approver/processList', { approverId: window.approverId })
			.done(function(data) {
				...
				var proc = list.map(function(doc) {
					console.log("처리 전 doc:", doc)
					doc.userStatus = '진행중';
					// 서버가 내려오는 필드 이름과 정확히 일치하는지 확인 (대소문자 포함)
					// 다음 결재자 정보가 객체로 내려온 경우 처리
					if (doc.nextApprover) {
						doc.approverName = doc.nextApprover.name || doc.approverName;
						doc.approverRankName = doc.nextApprover.rank || doc.approverRankName;
					}
				   console.log("처리 후 doc:", doc);
				   return doc;
				});
				renderTable(proc);
			}).fail(function() {
        ...
        rows = data.map(function(doc) {
            ...
            switch (doc.userStatus) {
                case '승인':
                    ...
                    break;
                case '진행중': {
                    // "진행중" 배지 우측에 다음 결재자 정보가 있으면 추가
                    // 예: "진행중 (이진우 / 부장)"
                    let base = `<span class="badge badge-status bg-info">진행중</span>`;
				    // 다음 결재자 정보가 있으면 강조 스타일로 추가
                    if (doc.nextApproverName) {
						base += `<span class="next-approver-arrow">
		                           <i class="fas fa-arrow-right"></i> ${doc.nextApproverName} ${doc.nextApproverRank ? ' / ' + doc.nextApproverRank : ''}
		                         </span>`
                    }
                    label = base;
                    break;
            ...
            // 결재자 칸은 기존대로 “doc.approverName” 등을 표시
            const approverInfo = doc.approverName || '-';
