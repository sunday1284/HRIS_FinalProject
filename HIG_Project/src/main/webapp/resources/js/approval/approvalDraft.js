...
$(document).ready(function() {
	...
	// 현재 연도를 기준으로 시작과 종료 범위 설정 (월은 0부터 시작하므로 5월은 4, 12월은 11)
	const currentYear = new Date().getFullYear();
	const minStartDate = new Date(currentYear, 4, 1);    // 5월 1일
	const maxStartDate = new Date(currentYear, 11, 30);  // 12월 30일

	// 두 날짜 사이의 랜덤 날짜를 생성하는 함수
	...
	// 연차 시작일을 랜덤으로 생성하고, 종료일은 시작일의 바로 다음 날로 설정하는 함수
	function generateRandomLeaveDates() {
	  const randomStartDate = getRandomDate(minStartDate, maxStartDate);
	  // 종료일은 시작일의 바로 다음 날 (2일간의 연차)
	  const randomEndDate = new Date(randomStartDate);
	  randomEndDate.setDate(randomStartDate.getDate() + 1);
	  return {
	    start: formatDateInput(randomStartDate),
	    end: formatDateInput(randomEndDate)
	  };
	}

	// 날짜 객체를 "YYYY-MM-DD" 형식 문자열로 변환하는 헬퍼 함수
	...
	// CKEditor 혹은 textarea에 일괄 사유 문구 세팅
	function setFormattedReason(text) {
	  if (window.CKEDITOR && CKEDITOR.instances.reason) {
	    CKEDITOR.instances.reason.setData(text);
	  } else {
	    const ta = document.querySelector("#reason");
	    if (ta) ta.value = text;
	  }
	}

	// 오늘 날짜를 "YYYY년 M월 D일" 포맷으로 반환
	function getFormattedToday() {
	  const d = new Date();
	  return `${d.getFullYear()}년 ${d.getMonth()+1}월 ${d.getDate()}일`;
	}

	// 기안자 이름 가져오기 (fn_approverProcess 가 로드한 window.approverData 기준)
	function getFirstDraftName() {
	  return (window.approverData && window.approverData.length>0)
	    ? window.approverData[0].approverName
	    : "";
	}
	
	//결재자 이름 가져오기 
	function getFirstApproverName() {
	  const firstInput = document.querySelector(
	    "#approverTableBody .approverName"
	  );
	...
	// [추가] 연차 자동 입력 버튼 이벤트 등록
	    ...
	    if (autoFillBtn) {
			autoFillBtn.addEventListener("click", function() {
			    // 1) 랜덤 날짜 생성
			    const { start: rs, end: re } = generateRandomLeaveDates();
			    document.querySelector("#leaveDate").value    = rs;
			    document.querySelector("#leaveEndDate").value = re;

			    // 2) 연차 유형(select)에 기본값 한 번만 설정
			    const defaultAnnualCode = "AC_03"; 
			    const annualCodeSelect = document.querySelector("#annualCode");
			    if (annualCodeSelect) {
			        annualCodeSelect.value = defaultAnnualCode;
			        // 혹시 change 리스너가 붙어있다면 강제로 트리거
			        annualCodeSelect.dispatchEvent(new Event("change", { bubbles: true }));
			    }

			    // 3) “사유” 필드 포맷 예시
			    const applicant  = getFirstDraftName();
			    const reasonText = "가족 여행을 위한 연차 신청입니다.";  // 원하시는 사유
			    const today      = getFormattedToday();        // "2025년 4월 17일"
			    const approver   = getFirstApproverName();     // 결재자 이름 
				const formatted = `
				<p>[연차 신청]</p>

				<p>상기 ${applicant} 본인은 ${reasonText} 이유로 연차를 신청하고자 하오니,<br>
				승인을 요청드립니다.</p>
				...
				<p>${today} 결재자 -> ${approver}</p>
				`;


			    setFormattedReason(formatted);
...
function expandCurrentDepartment() {
	const currentDepartmentId = document.getElementById("departmentId").value;
	    if (!currentDepartmentId) {
	        console.warn("현재 기안자의 departmentId가 비어있습니다.");
	        return;
	    }
	    // 좌측 조직도
	    let orgTree = $.ui.fancytree.getTree("#orgTree");
	    if (orgTree) {
	        orgTree.visit(function(node) {
	            if (node.folder && node.key.startsWith("dept_") && node.data &&
	                node.data.departmentId == currentDepartmentId) {
	                node.setExpanded(true);
	                node.visitParents(function(parent) { parent.setExpanded(true); });
	                node.visit(function(child) { child.setExpanded(true); });
	                node.makeVisible();
	    ...
	    // 우측 즐겨찾기 트리
	    let favTree = $.ui.fancytree.getTree("#favoriteOrgTree");
	    if (favTree) {
	        favTree.visit(function(node) {
	            if (node.folder && node.key.startsWith("dept_") && node.data &&
	                node.data.departmentId == currentDepartmentId) {
	                node.setExpanded(true);
	                node.visitParents(function(parent) { parent.setExpanded(true); });
...
function openOrgModal() {
	...
	axios.get('/approval/employee/getLineApprovers', { params: { empId: currentEmpId } })
		.then(function(response) {
			...
			if (tree) {
				expandAndHighlightCurrentTeamOrUser(); // 현재 기안자의 부서만 확장하도록 호출
			}
...
function expandAndHighlightCurrentTeamOrUser() {
    ...
    if (!currentDepartmentId) {
        ...
        // 필요시 사용자에게 알림 표시
         showInfo({ title: "정보", text: "사용자의 부서 정보를 찾을 수 없어 부서 확장을 건너뜁니다." });
...
function searchAndExpandTree(keyword) {
    ...
    let found = false;
    // 검색할 트리 선택자 배열 (필요시 다른 트리도 추가 가능)
    const treeSelectors = ["#orgTree", "#favoriteOrgTree"];
    
    treeSelectors.forEach(selector => {
        const tree = $.ui.fancytree.getTree(selector);
        if (!tree) return;

        // 기존에 하이라이트했던 배경색 초기화 (새 검색 시 초기화)
        tree.visit(function(node) {
            $(node.span).css("background", "");
        });
    ...
    if (!found) {
        showInfo({ title: "검색 결과", text: "검색 결과가 없습니다." });
    }
}
...
*/
// 우측 즐겨찾기 결재라인을 로드하는 함수
function loadFavoriteOrgTree() {
    // 1) 기존 트리 파괴
    let existingFavTree = $.ui.fancytree.getTree("#favoriteOrgTree");
    if (existingFavTree) { existingFavTree.destroy(); }
    
    const currentEmpId = document.getElementById("empId").value;
    axios.get("/approval/employee/getFavoriteApprovers", { params: { empId: currentEmpId } })
        .then(function(response) {
            // 서버에서 받은 전체 트리 데이터
            let favoriteTreeData = response.data;

            // 2) 첫 번째 ‘즐겨찾기’ 노드 제거 (만약 title이 "즐겨찾기"인 노드가 있다면)
            //    - title이 "즐겨찾기"인 최상위 노드를 찾아서 제거 (기존 코드에 맞춰서 작성)
            favoriteTreeData = favoriteTreeData.filter(node => node.title !== "즐겨찾기");

            // 3) 새로 만들 "즐겨찾기" 노드 (예: 두 번째 사진에 있는 구조)
            //    여기서는 title만 "즐겨찾기"로 고정, 나머지 dynamicDeptData에서 받아오는 예시
            let dynamicDeptData = {
                key: "dept_fav_RD",
                folder: true,
                children: [
                  {
                    ...
                    key: "2020010001",
                    folder: false,
                    data: { empId: "2020010001", rankName: "부장", departmentName: "경영지원본부" }
                  },
                  {
                    title: "김형태",
                    key: "2025032001",
                    folder: false,
                    data: { empId: "2025032001", rankName: "과장", departmentName: "생산본부" }
                  }
                ]
            };

            // title만 즐겨찾기로 고정하고, 다른 속성은 dynamicDeptData에서 가져옴
            const customDeptNode = {
                ...dynamicDeptData,
                title: "즐겨찾기"
            };

            // 4) 새 "즐겨찾기" 노드를 맨 앞으로(unshift) 삽입
            favoriteTreeData.unshift(customDeptNode);

            // 5) fancyTree 초기화
            $("#favoriteOrgTree").fancytree({
                icon: false,
                checkbox: true,
                selectMode: 3,
                glyph: {
                ...
                renderNode: function(event, data) {
                    ...
                    if (node.folder) {
                        ...
                        // 사람 노드
                        iconHtml = "<i class='fas fa-user'></i> ";
                        let rankName = node.data.rankName || "";
                        $title.html(iconHtml + node.title + (rankName ? " (" + rankName + ")" : ""));
                        $(node.span).off("click.toggleSelect").on("click.toggleSelect", function(e) {
                            if ($(e.target).hasClass("fancytree-checkbox")) return;
                            node.setSelected(!node.isSelected());
        ...
        .catch(function(error) {
            console.error("즐겨찾기 결재자 목록 로드 실패:", error);
            showError({ title: "오류", text: "즐겨찾기 결재자 목록을 불러오는데 실패했습니다." });
        });
}



// 모달 열 때 두 트리 모두 로드하는 함수: 좌측 orgTree와 우측 favoriteOrgTree
function openOrgModalDualView() {
    // 기존 좌측 조직도는 openOrgModal() 호출 (기존 함수 그대로 사용)
    openOrgModal();
    // 우측 즐겨찾기 로드
    loadFavoriteOrgTree();
    // 모달 표시 (이미 orgModal 열릴 예정)
    let modal = new bootstrap.Modal(document.getElementById("orgModal"));
    modal.show();
}
...
// 저장시: 좌측과 우측 둘 다 선택한 노드를 합쳐서 새로운 결재자를 구성
function saveApprovalLineFromOrg() {
    console.log("saveApprovalLineFromOrg 호출됨");
    let leftTree = $.ui.fancytree.getTree("#orgTree");
    let leftSelected = leftTree ? leftTree.getSelectedNodes() : [];
    let rightTree = $.ui.fancytree.getTree("#favoriteOrgTree");
    let rightSelected = rightTree ? rightTree.getSelectedNodes() : [];
    // 두 영역의 선택 노드 취합 (중복이 있을 경우 걸러내면 좋겠으나, 여기서는 단순 합산)
    let allSelectedNodes = [...leftSelected, ...rightSelected];
