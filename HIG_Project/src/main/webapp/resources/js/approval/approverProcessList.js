 ...
 *   수정일        수정자           수정내용
 *  -----------  -------------    ---------------------------
 * 2025. 4.  1.  CHOI            결재진행함 뷰단 및 필터링 로직 수정
 * </pre>
 */
...
function fetchApproverProcessList(){
    ...
    $.ajax({
        url: "/approval/approver/processList",
        type: "GET",
        ...
        error: function(xhr, status, error){
            ...
            $("#approverProcessListContainer").html(`<tr><td colspan="10">문서 조회 중 오류가 발생했습니다.</td></tr>`);
        }
    });
}
...
function renderApproverProcessList(data) {
    ...
    $("#approverProcessListContainer").html(html);
    initializeDataTable();
}
...
function initializeDataTable() {
    ...
    if ($.fn.DataTable && $.fn.DataTable.isDataTable('#approverTable')) {
        $('#approverTable').DataTable().destroy();
    }
    const dataTable = new simpleDatatables.DataTable("#approverTable", {
        searchable: true,
        fixedHeight: false
    });
